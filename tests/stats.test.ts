...
describe("GET /orgs/stats", () => {
  ...
  describe("filters", () => {
    ...
    it("forwards explicit filters to providers", async () => {
      mockFetch.mockImplementation((url: string) => {
        if (url.includes("3010")) return Promise.resolve(mockPostmarkStats());
        if (url.includes("3011")) return Promise.resolve(mockInstantlyStats());
        return Promise.reject(new Error("Unexpected URL"));
      ...
      await authedGet("/orgs/stats?brandId=brand_1&featureSlugs=feat_1");
  ...
  describe("groupBy", () => {
    ...
    it("passes through distinct recipient groups without changing provider counts", async () => {
      mockFetch.mockResolvedValueOnce(
        mockGroupedInstantly([
          {
            key: "a@acme.com",
      ...
      const res = await authedGet("/orgs/stats?type=broadcast&brandId=brand_1&featureSlugs=active_goal_feature&groupBy=recipientEmail");

      expect(res.status).toBe(200);
      expect(res.body.groups).toHaveLength(2);

      const byKey = new Map(res.body.groups.map((g: { key: string }) => [g.key, g]));
      expect(byKey.get("a@acme.com").broadcast.recipientStats.clicked).toBe(4);
      expect(byKey.get("a@acme.com").broadcast.recipientStats.repliesPositive).toBe(2);
      expect(byKey.get("a@acme.com").broadcast.emailStats.clicked).toBe(5);
      expect(byKey.get("b@acme.com").broadcast.recipientStats.clicked).toBe(1);
      expect(byKey.get("b@acme.com").broadcast.recipientStats.repliesPositive).toBe(0);

      const params = new URL(mockFetch.mock.calls[0][0]).searchParams;
      expect(params.get("groupBy")).toBe("recipientEmail");
      expect(params.get("brandId")).toBe("brand_1");
